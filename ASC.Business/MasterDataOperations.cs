       ...
       public async Task<bool> UploadBulkMasterData(List<MasterDataValue> values)
{
    try
    {
        using (_unitOfWork)
        {
            ...
            {
                Console.WriteLine($"Processing MasterDataValue: PartitionKey={value.PartitionKey}, Name={value.Name}, IsActive={value.IsActive}");
    ...
    {
        Console.WriteLine($"UploadBulkMasterData error: {ex.Message}, StackTrace: {ex.StackTrace}");
        return false;
    }
}
    }
